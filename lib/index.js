...
var SkillManager = class _SkillManager {
  ...
  async emitPlayerEvent(playerId, eventType) {
    ...
    const playerData = werewolfGameData.playersData.find(
      (pd) => pd.player.id === playerId
    );
    if (!playerData?.role) {
...
// src/game/ingame/InGameManager.ts
import { KairoUtils as KairoUtils2 } from "@kairo-js/router";
...
import { KairoUtils as KairoUtils3 } from "@kairo-js/router";
  ...
  handleWorldStateChange(newState, ingameConstants) {
    switch (newState) {
      case SCRIPT_EVENT_MESSAGES.IN_GAME:
        this.systemManager.changeWorldState(
          "InGame" /* InGame */,
          ingameConstants
        );
        break;
...
var DefinitionRegistry2 = class _DefinitionRegistry {
  constructor(systemManager) {
    this.systemManager = systemManager;
    this.state = {};
  }
...
var SystemManager = class _SystemManager {
  constructor() {
    ...
    this.registry = DefinitionRegistry2.create(this);
  ...
  async handlePlayerSkillTrigger(playerId, eventType) {
    if (!this.inGameManager)
      return KairoUtils4.buildKairoResponse(
        {},
        false,
        "InGameManager is not initialized"
      );
